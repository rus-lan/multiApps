...
5. Adds `CLAUDE.md` and `AGENTS.md` to each clone's `.git/info/exclude`, so those files never reach the app's own remote.
6. Regenerates the root `Makefile` and creates `mk/` if missing.
7. Writes the three wrapper prompt files (see below).
8. Creates or updates `.gitignore` to ignore `apps/`.
9. Runs `git init` in the workspace root if it is not a git repo yet.
